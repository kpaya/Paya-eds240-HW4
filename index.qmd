...
Here, I will explore the question: *How does drought impact the economy of a small community?* The data analyzed focuses on the effects of the 2022 drought, specifically the volume and cost of water purchased. These datasets were sourced from the California Department of Water Resources and the Nasdaq Veles California Water Index. By examining Coalinga’s water purchases, price fluctuations, and regional drought reports, I aim to better understand the pressures faced by local water systems. Through visualizations, I will highlight how smaller communities are disproportionately impacted by environmental challenges and emphasize the broader economic consequences of water scarcity.

### Data Details

I accessed monthly state water shortage levels by urban water suppliers using data from the California Department of Water Resources. To determine the price per acre-foot of water, I accessed historical data from the Nasdaq Veles California Water Index (NQH2O).
...
bar_graph <- ggplot(city_data, aes(x = state_standard_shortage_level, y = supplier_name, fill = factor(state_standard_shortage_level))) +
  ...
  # Manually specify colors for each of the 5 levels
  scale_fill_manual(
    values = c("1" = "lightgoldenrod", "2" = "gold", "3" = "orange", "4" = "orange3", "5" = "firebrick"),
    labels = c("Low Risk", "Medium-Low Risk", "Medium Risk", "Medium-High Risk", "High Risk")
  ) +  
  ...
  theme(
    ...
    plot.background = element_rect(fill = "#FFF1E0", color = "#FFF1E0"),  # Change background color
...
timeseries <- ggplot(waterprice, aes(x = Date, y = price)) +
  ...
  geom_line(color = "orange3") +  # Add the line plot on top
  ...
  # Add the "Drought Event" label as a bracket instead of a filled rectangle
  annotate("text", x = as.Date("2022-06-01"), y = max(waterprice$price) * 0.9, label = "Drought Event", 
           vjust = -10, hjust = 3, color = "orange", size = 4) +
  
  # Create the bracket using geom_segment
  geom_segment(aes(x = as.Date("2022-01-01"), y = max(waterprice$price) * 1.05, 
                   xend = as.Date("2022-12-31"), yend = max(waterprice$price) * 1.05),
               color = "black", size = 1.2, lineend = "round") +  # The bracket line
  
  # Add vertical lines to create the look of a bracket
  geom_segment(aes(x = as.Date("2022-01-01"), y = max(waterprice$price) * 1.05, 
                   xend = as.Date("2022-01-01"), yend = max(waterprice$price) * 1),
               color = "black", size = 1.2, lineend = "round") + 
  
  geom_segment(aes(x = as.Date("2022-12-31"), y = max(waterprice$price) * 1.05, 
                   xend = as.Date("2022-12-31"), yend = max(waterprice$price) * 1),
               color = "black", size = 1.2, lineend = "round") + 

  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),  # Center and bold the title
    plot.background = element_rect(fill = "#FFF1E0", color = "#FFF1E0"),  # Change background color to match the other graph
    panel.grid.major = element_blank(),  # Remove major grid lines
    panel.grid.minor = element_blank()   # Remove minor grid lines
  )
```
